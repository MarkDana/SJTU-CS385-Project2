...
class SegResNet101(BaseModel):
    def __init__(self, num_classes, in_channels=3, pretrained=True, freeze_bn=False, **_):
        super(SegResNet101, self).__init__()
        resnet101 = models.resnet101(pretrained=pretrained)
        encoder = list(resnet101.children())
        if in_channels != 3:
            encoder[0] = nn.Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=1)
        encoder[3].return_indices = True
        ...
        # Encoder
        self.first_conv = nn.Sequential(*encoder[:4])
        resnet101_blocks = list(resnet101.children())[4:-2]
        self.encoder = nn.Sequential(*resnet101_blocks)

        # Decoder
        resnet101_untrained = models.resnet101(pretrained=False)
        resnet101_blocks = list(resnet101_untrained.children())[4:-2][::-1]
        decoder = []
        channels = (2048, 1024, 512)
        for i, block in enumerate(resnet101_blocks[:-1]):
            new_block = list(block.children())[::-1][:-1]
            decoder.append(nn.Sequential(*new_block, DecoderBottleneck(channels[i])))
        new_block = list(resnet101_blocks[-1].children())[::-1][:-1]
        decoder.append(nn.Sequential(*new_block, LastBottleneck(256)))
