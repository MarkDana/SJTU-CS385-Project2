...
class FCN8_BN(BaseModel):
    def __init__(self, num_classes, pretrained=True, freeze_bn=False, **_):
        ...
        # Enable ceil in max pool, to avoid different sizes when upsampling
        for layer in features:
            if 'MaxPool' in layer.__class__.__name__:
                layer.ceil_mode = True

        # Extract pool3, pool4 and pool5 from the VGG net
        self.pool3 = nn.Sequential(*features[:23])
        self.pool4 = nn.Sequential(*features[23:33])
        self.pool5 = nn.Sequential(*features[33:])
